...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [_BL] = LAYOUT_ortho(
                  ...
                  KC_LSFT,  KC_TAB,    KC_ENT,     KC_LALT,
                  KC_LCTL,  KC_DEL,    KC_GRV,     KC_SPC
                 ),

    [_ML] = LAYOUT_ortho(
                  TO(_BL), 
                  MI_VELD,  MI_VELU,  MI_OCTD, MI_OCTU,
...
void slider(void) {
    ...
    current_read = (analogReadPin(SLIDER_PIN) +last_read)/8; //filter strength
 
    if (current_read != last_read ) { // (&midi_device, chan, message, max control value - (current pin reading) >>resolution)
        midi_send_cc(&midi_device, 1, 0x19, (analogReadPin(SLIDER_PIN) >>3));
                                // channel 1, CC 25, (127 - sliderValue), resolution of 3
...
bool wpm_keycode_user(uint16_t keycode) {
    ...
    if ((keycode >= KC_A && keycode <= KC_0) || (keycode >= KC_TAB && keycode <= KC_SLSH) || (keycode >= KC_HOME && keycode <= KC_UP) || (keycode >= MI_C && keycode <= MI_OCTU)) {
        return true;
    }
...
#    define IDLE_SPEED 2  // below this wpm value your animation will idle

// #define PREP_FRAMES 1 // uncomment if >1

#    define TAP_FRAMES 2
#    define TAP_SPEED 3  // above this wpm value typing animation to trigger
